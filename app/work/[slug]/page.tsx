    ...
    <article className="px-6 pt-32 pb-20 max-w-content mx-auto">
      <div className="mb-12">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <span className="font-mono text-small text-text-secondary border-r border-border pr-4">{work.role}</span>
          {work.stack.map((tag) => (
            <span key={tag} className="font-mono text-small text-text-tertiary border border-border px-2 py-0.5">{tag}</span>
          ))}
        </div>
      </div>
